...
Note: Since `.txt` files cannot include images, adjust font sizes, or set headings, please refer to `README.md` for a clearer and more detailed presentation.
...
2. Setup
1. Upload the Notebook to Colab: Open Google Colab notebook and upload the Yelp_NJ.ipynb file.
2. Upload or Mount Data:
o If your dataset is stored locally, upload it directly into the Colab session by navigating to Files > Upload.
o Alternatively, if you are working with a dataset on Google Drive, mount your Google Drive to access the data directly. Run the following code cell in Colab to mount your drive:
...
o Data Import: Loads the Yelp data from the specified source.
o Data Preprocessing: Cleans and organizes the data.
o Exploratory Data Analysis (EDA): Visualizes trends, distributions, and patterns in Yelp reviews.
...
2. Running Comprehensive Text Analysis on Google Colab
This guide explains how to run a comprehensive text analysis script on Google Colab. The script analyzes multiple text files (from StackOverflow, Reddit, news articles, and patents) for readability, structural elements, sentiment, and use of technical terms. This process provides insights into the writing style differences across these text types. You can start by opening this Google Colab notebook.
Instructions
...
Note: The data and the program are available at Kaggle: Yelp NJ Dataset.  Also, we provide two different application versions: search_engine.exe for Windows and search_engine for macOS.
Prerequisites
...
o termcolor
- Data and Program:
o Download from Kaggle: Yelp NJ Dataset
...
- Visit the Kaggle: Yelp NJ Dataset page.
- Download the dataset and program files.
- Extract the files to a directory on your machine.
Step 2: Install Python 3
If you don't have Python 3 installed, download it from the official website and follow the installation instructions for your operating system.
...
- Instructions:
4. Choose option 2.
5. Enter keywords or phrases when prompted.
§ Use the same format as in Option 1.
6. Enter the number of top results to return.
- Example:
...
- Instructions:
7. Choose option 3.
8. Enter the minimum and maximum latitude and longitude values when prompted.
§ Latitude Range: Between 39.05 and 40.39.
§ Longitude Range: Between -75.55 and -74.66.
9. Enter the number of top results to return.
- Example:
...
- Instructions:
10. Choose option 4.
11. You will see a sub-menu with two options:
  Options:
...
- Instructions:
o Choose sub-option 1.
o A scatter plot will be displayed showing the distribution.
o Note: Ensure that your environment supports GUI display.
- Example:
...
o Number of reviews.
o Bounding box of the businesses reviewed.
o Top 10 most frequent words.
o Top 10 most frequent phrases.
o Three most representative sentences.
- Instructions:
...
Option Descriptions
1. View all positive comparisons: Displays the first 10 positive comparisons in a tabular format.
2. View all negative comparisons: Displays the first 10 negative comparisons in a tabular format.
3. Filter by keyword: The tool prompts for a keyword.
Example: Enter not as to see all comparisons containing this phrase.
4. Filter by subject: The tool prompts for a subject.
Example: Enter pizza to display all comparisons where either subject_a or subject_b is "pizza".
5. Display a range of rows: The tool prompts for a start and end row number.
Example: Enter 10 and 20 to display rows from 10 to 20 in the dataset.
6. Exit: Closes the application with a goodbye message.
...
- Results:
o The system returned 5 business listings that match the keywords.
o Rank: Indicates the position of the result based on relevance.
o Score: A numerical value representing how closely the business name matches the search terms.
o DocID: The internal document ID for the business in the index.
o Snippet: The name of the business, potentially with matched keywords highlighted.
- Observations:
o All the top-ranked businesses have the keywords "Chinese Food" in their names.
o The first four results have identical scores, indicating similar relevance.
o The fifth result has a slightly lower score but still contains the keywords.
...
- Top Results Requested: The user requested the top 5 results.
- Results:
o Similar to the previous sample, the results include businesses whose names contain the exact phrase "Chinese Food".
o The consistent scores suggest that the exact phrase "Chinese Food" was found in the business names.
- Observations:
o The results are the same as in the keyword search, indicating that in this case, the keyword and exact phrase searches yielded the same top results.
o This suggests that the phrase "Chinese Food" is a common exact match in business names within the dataset.
...
- Results:
o The system returned 5 reviews containing the keywords "Chinese" and "Food".
o Scores are slightly higher than those in the business search, possibly due to the lengthier content in reviews.
o Snippets:
§ Each snippet includes portions of the review text where the keywords appear.
§ Ellipses (...) indicate omitted text for brevity.
- Observations:
o The reviews discuss experiences with Chinese food, including opinions on authenticity, quality, and service.
o The snippets show varied contexts in which "Chinese food" appears.
...
- Top Results Requested: The user requested the top 5 results.
- Results:
o The results are identical to those from the keyword search in Sample 3.
o This indicates that the exact phrase "Chinese Food" frequently appears in the reviews.
- Observations:
o The exact phrase search did not narrow down the results further compared to the keyword search.
o Users often mention "Chinese food" as a phrase in their reviews.
...
- Bounding Box Input:
o Minimum Latitude: 39.10
o Maximum Latitude: 40.39
o Minimum Longitude: -75.58
o Maximum Longitude: -74.66
- Top Results Requested: The user requested the top 5 results.
- Results:
o The system returned 5 businesses located within the specified geographical bounding box.
o Rank: All results have the same score and are ranked based on their order in the index.
o Score: A uniform score of 2.0 indicates that the ranking is solely based on the spatial query without additional weighting.
o Business Details:
§ Name: The name of the business.
§ Latitude and Longitude: The geographical coordinates of the business.
- Observations:
o The businesses returned span various industries, indicating a broad search.
o The geographical coordinates of each business fall within the specified ranges.
...
2. Get User's Review Summary
Enter your choice (1 or 2): 1
System Output: （A Distribution of reviews contributed by user）
...
- Results:
o The displayed graph is Distribution of reviews contributed by users.
o The x-axis represents the Number of reviews contributed by users.
o The y-axis represents the Number of users having a specific number of reviews.
o Each red dot on the graph represents the count of users that have contributed a specific number of reviews. For instance, a large number of users have contributed just a few reviews, as shown by the dot at the 5-reviews mark, and there are fewer users as the number of reviews increases.
- Observations:
o The graph shows a clear trend where the number of users contributing reviews decreases as the number of reviews increases. This is typical in many review systems where most users contribute only a few reviews.
o The highest point on the left indicates that the majority of users have contributed only one review, which is common in user-generated content platforms.
- Plot Characteristics:
o Scatter Plot: The use of a scatter plot is appropriate here as it helps to visualize the distribution of discrete counts (number of reviews) among a population (users).
o Data Points: The specific counts, such as for 0, 5, 10, 15, 20, 25, 30, 35, and 40 reviews, help identify common patterns of user engagement on the platform.
...
- System Response:
o The system promptly informs the user that the user ID does not exist.
o Provides a helpful suggestion by offering two alternative, random user IDs that the user can try to retrieve summaries for, enhancing user experience by reducing frustration and guiding the user towards valid queries.
- Observations:
o This feature shows the robustness of the system in handling errors and guiding the user to rectify them.
o Providing alternative IDs helps ensure that users can still engage with the system even if they initially make a mistake.
...
- Results:
o The user has contributed 99 reviews.
o Bounding Box: Lists addresses of businesses reviewed, showcasing geographical diversity in the user's reviews.
o Top-10 Most Frequent Words and Phrases: Highlights common words and phrases, offering insights into the user's writing style and frequent topics.
o Representative Sentences: Provides direct examples of the user's review content, giving a snapshot of their perspectives and review contexts.
- Observations:
o Detailed user review summaries like this can help in understanding individual user behavior, preferences, and their engagement with various businesses.
o This feature is useful for market analysis, customer relationship management, and personalizing user experiences based on review patterns.
...
- System Response:
o The system acknowledges the user's choice to exit by displaying a simple farewell message, "Bye!".
- Observations:
o This feature demonstrates the system's ability to terminate sessions effectively, respecting the user's intention without additional prompts or delays, thereby enhancing the user experience.
- Process:
o After displaying the farewell message, the system would typically terminate the session, closing any open resources or saving necessary states as configured.
